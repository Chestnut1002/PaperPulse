...
2. **YAML 八进制陷阱**:数据库密码 `051002` 在 YAML 中不加引号会被解析为**八进制整数 20994**,导致 `Access denied for user 'root'@'localhost' (using password: YES)`。现象具有迷惑性——profile 已正确激活、配置文件确实加载,只有值被静默改写
...
2. 密码值加引号:`password: "051002"`。**凡是以 0 开头的数字型字符串(密码、手机号、学号、编号)在 YAML 中一律加引号**
